<template>
    <div class="testimonios__list max-w-full p-32 lg:px-20 lg:pb-0 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-4"
        v-if="data">
        <article v-for="testimonio in data" :key="testimonio.id"
            class="card item rounded-xl overflow-hidden min-h-[600px] flex flex-col justify-center items-center gap-2 lg:gap-8 bg-nude-6 p-0">
            <nuxt-link :to="formatPermalink(testimonio.permalink)" class="size-full">
                ...
                <div
                    ...
                    <h3 class="h6 text-clamp-base lg:text-clamp-lg font-medium">{{ testimonio.post_title }}</h3>
                    <div
