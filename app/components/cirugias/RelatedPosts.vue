                ...
                <swiper-container ref="containerRef" :init="false"
                    ...
                    <swiper-slide v-for="post in posts" :key="post.id"
                        ...
                        <nuxt-link v-if="post.title && post.title.rendered" :to="`/blog/${post.slug}`" :aria-label="post.title ? 'Leer más sobre ' + post.title.rendered : 'Leer más'
                            ...
                            <UiImage loading="lazy" :data="post"
                                class="rounded-2xl size-full aspect-square object-cover" />
                            <h2 class="bg-white rounded-2xl p-4 mt-4 text-2xl text-pretty"
                                v-if="post.title && post.title.rendered" :id="'post-title-' + post.id">
                                {{ post.title.rendered }}
                            </h2>
                        </nuxt-link>
                    </swiper-slide>
