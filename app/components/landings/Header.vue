<template>
   <div class="hero__intro bg-cover bg-center w-full lg:w-1/2 min-h-vh/90 overflow-hidden flex flex-col justify-end">
      ...
      <header
         ...
         ref="cabecera">
         <nuxt-link to="/">
            <img class="header__landing-logo max-w-20 md:max-w-16" src="/assets/images/icons/logo-landings.svg" alt=""
      ...
      <div
         class="header__landing-content text-nude-8 text-center p-5 md:p-24 pb-24 size-full flex flex-col justify-end items-center">
         <h1 class="header__landing-content-title text-clamp-4xl [&>span]:span-gradient [&>span]:border-b-[1px] [&>span]:border-b-white/20 [&>span]:block"
            v-html="data.acf.titulo_landing"></h1>
         <div class="flex flex-col items-center gap-4 w-full">
            <!-- <ElementsPlazas v-if="data.acf.plazas?.numero_plazas" :data="data.acf.plazas" /> -->
            <ElementsButton v-if="data.acf && data.acf.quiz_multiple && data.acf.quiz_multiple.multiple_forms"
               class="gold text-clamp-sm uppercase text-center w-fit" href="#presupuesto">
               Calcula tu presupuesto
            </ElementsButton>
            <div v-if="data.acf && data.acf.precio" class="precio flex flex-col items-center w-full">
               <p v-if="data.acf.opciones_precio === 'mes'" class="h3 [&>span]:span-gradient">desde <span>{{
                     data.acf.precio }}€/mes</span></p>
               <p v-else-if="data.acf.opciones_precio === 'dia'" class="h3 [&>span]:span-gradient">desde <span>{{
                     data.acf.precio }}€/día</span></p>
               <p v-else-if="data.acf.opciones_precio === 'desde'" class="h3 [&>span]:span-gradient">desde <span>{{
                     data.acf.precio }}€</span></p>
