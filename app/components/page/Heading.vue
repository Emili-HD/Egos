<template>
    <header v-if="data"
        class="heading__cirugias h-vh px-8 xl:px-16 py-6 xl:py-12 mb-0 col-start-1 col-span-full xl:col-span-9 flex justify-center items-end bg-cover bg-center"
        ...
        <div class="header__content pb-8 relative z-10">
            ...
            <h1 v-if="data.title && data.title.rendered"
                class="text-nude-8 text-6xl mb-10 text-clamp-6xl text-balance text-center">
                <u>{{ data.title.rendered }}</u>
            </h1>
            <img v-if="data.acf && data.acf.logo" :src="data.acf.logo.url" width="270" height="44" alt="Bloome by Egos"
                class="mx-auto">
        </div>
    </header>
    <div v-if="data.acf && data.acf.formulario"
        class="form__wrapper min-h-[620px] h-vh bg-blue-1 [.blackfriday_&]:!bg-blackfriday pt-16 col-start-1 col-span-full xl:col-start-10 xl:col-span-7 flex flex-col justify-center items-stretch [.estetica:not(.blackfriday)_&]:bg-crema">
